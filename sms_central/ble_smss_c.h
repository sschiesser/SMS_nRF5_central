...
#ifndef BLE_SMSS_C_H__
#define BLE_SMSS_C_H__
...
typedef enum
{
    BLE_SMSS_C_EVT_DISCOVERY_COMPLETE = 1,  /**< Event indicating that the LED Button Service has been discovered at the peer. */
    BLE_SMSS_C_EVT_BUTTON_NOTIFICATION,      /**< Event indicating that a notification of the LED Button Button characteristic has been received from the peer. */
	BLE_SMSS_C_EVT_PRESSURE_NOTIFICATION,
	BLE_SMSS_C_EVT_IMU_NOTIFICATION
} ble_smss_c_evt_type_t;
...
typedef struct
{
    ble_smss_c_evt_type_t evt_type;    /**< Type of the event. */
    ...
    union
    {
		...
		ble_pressure_t pressure;
		ble_imu_t imu;
        smss_db_t     peer_db;         /**< LED Button Service related handles found on the peer device. This will be filled if the evt_type is @ref BLE_LBS_C_EVT_DISCOVERY_COMPLETE.*/
    } params;
} ble_smss_c_evt_t;


// Forward declaration of the ble_lbs_c_t type.
typedef struct ble_smss_c_s ble_smss_c_t;
 ...
 */
typedef void (* ble_smss_c_evt_handler_t) (ble_smss_c_t * p_ble_smss_c, ble_smss_c_evt_t * p_evt);
...
struct ble_smss_c_s
{
    ...
    smss_db_t                peer_smss_db;  /**< Handles related to LBS on the peer*/
    ble_smss_c_evt_handler_t evt_handler;  /**< Application event handler to be called when there is an event related to the LED Button service. */
...
typedef struct
{
    ble_smss_c_evt_handler_t evt_handler;  /**< Event handler to be called by the LED Button Client module whenever there is an event related to the LED Button Service. */
} ble_smss_c_init_t;
